...
void GCode::_do_export(Print& print, GCodeOutputStream &file, ThumbnailsGeneratorCallback thumbnail_cb)
{
    ...
    if(!has_BTT_thumbnail && !is_griffin_flavor(print.config().gcode_flavor.value)){
...
void GCode::print_machine_envelope(GCodeOutputStream &file, Print &print)
{
    ...
    if ((flavor == gcfMarlinLegacy || flavor == gcfMarlinFirmware || flavor == gcfRepRapFirmware
        ...
        if (flavor == gcfCheetah) {
            ...
            file.write_format("M215 X%.2lf Y%.2lf ; sets the jerk limits, m/s^3\n",
                print.config().machine_max_jerk_x.values.front() * 1000.0,
                print.config().machine_max_jerk_y.values.front() * 1000.0);
        } else {
...
// Griffin/Cheetah structured header generation
void GCode::write_griffin_header(GCodeOutputStream &file, Print &print, unsigned int first_printing_extruder_id)
{
    const auto flavor = print.config().gcode_flavor.value;
    const char *flavor_str = (flavor == gcfCheetah) ? "Cheetah" : "Griffin";

    file.write_format(";START_OF_HEADER\n");
    file.write_format(";HEADER_VERSION:0.1\n");
    file.write_format(";FLAVOR:%s\n", flavor_str);
    file.write_format(";GENERATOR.NAME:OrcaSlicer\n");
    file.write_format(";GENERATOR.VERSION:%s\n", SoftFever_VERSION);
    file.write_format(";GENERATOR.BUILD_DATE:%s\n", Slic3r::Utils::utc_timestamp(Slic3r::Utils::get_current_time_utc()).c_str());
    file.write_format(";TARGET_MACHINE.NAME:%s\n", print.config().printer_model.value.c_str());

    // Per-extruder metadata
    size_t num_extruders = print.config().nozzle_diameter.values.size();
    ...
    int bed_temp = get_bed_temperature(first_printing_extruder_id, true, bed_type);
    file.write_format(";BUILD_PLATE.INITIAL_TEMPERATURE:%d\n", bed_temp);

    // Build volume temperature (if applicable)
    if (print.config().machine_heated_build_volume.value) {
    ...
    for (const PrintObject *obj : print.objects()) {
        for (const PrintInstance &inst : obj->instances()) {
            BoundingBoxf3 obj_bb = obj->model_object()->bounding_box_exact();
            obj_bb.translate(inst.shift.x(), inst.shift.y(), 0.0);
            bbox.merge(obj_bb);
        }
    }
    if (bbox.defined) {
        file.write_format(";PRINT.SIZE.MIN.X:%.1f\n", bbox.min.x());
        file.write_format(";PRINT.SIZE.MIN.Y:%.1f\n", bbox.min.y());
        file.write_format(";PRINT.SIZE.MIN.Z:%.1f\n", bbox.min.z());
        file.write_format(";PRINT.SIZE.MAX.X:%.1f\n", bbox.max.x());
